...
UWheeledVehicleMovementComponent6W::UWheeledVehicleMovementComponent6W(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
...
#if WITH_EDITOR
void UWheeledVehicleMovementComponent6W::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
...
static void GetVehicle6WEngineSetup(const FVehicleEngineData& Setup, PxVehicleEngineData& PxSetup)
{
...
static void GetVehicle6WGearSetup(const FVehicleTransmissionData& Setup, PxVehicleGearsData& PxSetup)
{
...
static void GetVehicle6WAutoBoxSetup(const FVehicleTransmissionData& Setup, PxVehicleAutoBoxData& PxSetup)
{
	for (int32 i = 0; i < Setup.ForwardGears.Num(); i++)
	{
...
void SetupDriveHelper(const UWheeledVehicleMovementComponent6W* VehicleData, const PxVehicleWheelsSimData* PWheelsSimData, PxVehicleDriveSimDataNW& DriveData)
{

	PxVehicleEngineData EngineSetup;
	GetVehicle6WEngineSetup(VehicleData->EngineSetup, EngineSetup);
	...
	GetVehicle6WGearSetup(VehicleData->TransmissionSetup, GearSetup);
	DriveData.setGearsData(GearSetup);

	PxVehicleAutoBoxData AutoBoxSetup;
	GetVehicle6WAutoBoxSetup(VehicleData->TransmissionSetup, AutoBoxSetup);
	DriveData.setAutoBoxData(AutoBoxSetup);

}

static void CreateVehicleSimulation(PxVehicleWheelsSimData& Veh6WSimData, int32 NumWheels, PxVehicleDriveSimDataNW& DriveSimData6W)
{
	PxVehicleWheelsSimData* PWheelsSimData = PxVehicleWheelsSimData::allocate(4);
	Veh6WSimData.copy(*PWheelsSimData, 0, 0);
	Veh6WSimData.copy(*PWheelsSimData, 1, 1);
	Veh6WSimData.copy(*PWheelsSimData, 2, 2);
	Veh6WSimData.copy(*PWheelsSimData, 3, 3);
	//Copy front wheels bones to apply throttle and make rest of the wheels physical
	Veh6WSimData.copy(*PWheelsSimData, 0, 4);
	Veh6WSimData.copy(*PWheelsSimData, 1, 5); 
	Veh6WSimData.setTireLoadFilterData(PWheelsSimData->getTireLoadFilterData());
	...
	DriveSimData6W.setDiffData(DifData);
}

void UWheeledVehicleMovementComponent6W::SetupVehicle()
{
	...
	// Setup the wheels
	PxVehicleWheelsSimData* PWheelsSimData = PxVehicleWheelsSimData::allocate(6);

	// Setup drive data
	PxVehicleDriveSimDataNW DriveData;
	CreateVehicleSimulation(*PWheelsSimData, 6, DriveData);

	SetupWheels(PWheelsSimData);
	SetupDriveHelper(this, PWheelsSimData, DriveData);
	// Create the vehicle
	PxVehicleDriveNW* PVehicleDriveNW = PxVehicleDriveNW::allocate(6);
...
void UWheeledVehicleMovementComponent6W::UpdateSimulation(float DeltaTime)
{
	...
	TArray<FRichCurveKey> SteerKeys = SteeringCurve.GetRichCurve()->GetCopyOfKeys();
	const int32 MaxSteeringSamples = FMath::Min(6, SteerKeys.Num());
...
void UWheeledVehicleMovementComponent6W::UpdateEngineSetup(const FVehicleEngineData& NewEngineSetup)
{
#if WITH_VEHICLE
	if (PVehicleDrive)
	{
		PxVehicleEngineData EngineData;
		GetVehicle6WEngineSetup(NewEngineSetup, EngineData);
...
void UWheeledVehicleMovementComponent6W::UpdateTransmissionSetup(const FVehicleTransmissionData& NewTransmissionSetup)
{
#if WITH_VEHICLE
	if (PVehicleDrive)
	{
		PxVehicleGearsData GearData;
		GetVehicle6WGearSetup(NewTransmissionSetup, GearData);

		PxVehicleAutoBoxData AutoBoxData;
		GetVehicle6WAutoBoxSetup(NewTransmissionSetup, AutoBoxData);
