...
UWheeledVehicleMovementComponent12W::UWheeledVehicleMovementComponent12W(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
...
#if WITH_EDITOR
void UWheeledVehicleMovementComponent12W::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
...
static void GetVehicle12WEngineSetup(const FVehicleEngineData& Setup, PxVehicleEngineData& PxSetup)
{
...
static void GetVehicle12WGearSetup(const FVehicleTransmissionData& Setup, PxVehicleGearsData& PxSetup)
{
...
static void GetVehicle12WAutoBoxSetup(const FVehicleTransmissionData& Setup, PxVehicleAutoBoxData& PxSetup)
{
	for (int32 i = 0; i < Setup.ForwardGears.Num(); i++)
	{
...
void SetupDriveHelper(const UWheeledVehicleMovementComponent12W* VehicleData, const PxVehicleWheelsSimData* PWheelsSimData, PxVehicleDriveSimDataNW& DriveData)
{

	PxVehicleEngineData EngineSetup;
	GetVehicle12WEngineSetup(VehicleData->EngineSetup, EngineSetup);
	...
	GetVehicle12WGearSetup(VehicleData->TransmissionSetup, GearSetup);
	DriveData.setGearsData(GearSetup);

	PxVehicleAutoBoxData AutoBoxSetup;
	GetVehicle12WAutoBoxSetup(VehicleData->TransmissionSetup, AutoBoxSetup);
	DriveData.setAutoBoxData(AutoBoxSetup);

}

static void CreateVehicleSimulation12W(PxVehicleWheelsSimData& Veh12WSimData, int32 NumWheels, PxVehicleDriveSimDataNW& DriveSimData12W)
{
	PxVehicleWheelsSimData* PWheelsSimData = PxVehicleWheelsSimData::allocate(4);
	Veh12WSimData.copy(*PWheelsSimData, 0, 0);
	Veh12WSimData.copy(*PWheelsSimData, 1, 1);
	Veh12WSimData.copy(*PWheelsSimData, 2, 2);
	Veh12WSimData.copy(*PWheelsSimData, 3, 3);
	//Copy front wheels bones to apply throttle and make rest of the wheels physical
	Veh12WSimData.copy(*PWheelsSimData, 0, 4);
	Veh12WSimData.copy(*PWheelsSimData, 1, 5);
	Veh12WSimData.copy(*PWheelsSimData, 0, 6);
	Veh12WSimData.copy(*PWheelsSimData, 1, 7);
	Veh12WSimData.copy(*PWheelsSimData, 0, 8);
	Veh12WSimData.copy(*PWheelsSimData, 1, 9);
	Veh12WSimData.copy(*PWheelsSimData, 0, 10);
	Veh12WSimData.copy(*PWheelsSimData, 1, 11);
	Veh12WSimData.setTireLoadFilterData(PWheelsSimData->getTireLoadFilterData());
	...
	DriveSimData12W.setDiffData(DifData);
}

void UWheeledVehicleMovementComponent12W::SetupVehicle()
{
	...
	// Setup the wheels
	PxVehicleWheelsSimData* PWheelsSimData = PxVehicleWheelsSimData::allocate(12);

	// Setup drive data
	PxVehicleDriveSimDataNW DriveData;
	CreateVehicleSimulation12W(*PWheelsSimData, 12, DriveData);

	SetupWheels(PWheelsSimData);
	SetupDriveHelper(this, PWheelsSimData, DriveData);

	PxVehicleDriveNW* PVehicleDriveNW = PxVehicleDriveNW::allocate(12);
...
void UWheeledVehicleMovementComponent12W::UpdateSimulation(float DeltaTime)
{
	...
	TArray<FRichCurveKey> SteerKeys = SteeringCurve.GetRichCurve()->GetCopyOfKeys();
	const int32 MaxSteeringSamples = FMath::Min(12, SteerKeys.Num());
...
void UWheeledVehicleMovementComponent12W::UpdateEngineSetup(const FVehicleEngineData& NewEngineSetup)
{
#if WITH_VEHICLE
	if (PVehicleDrive)
	{
		PxVehicleEngineData EngineData;
		GetVehicle12WEngineSetup(NewEngineSetup, EngineData);
...
void UWheeledVehicleMovementComponent12W::UpdateTransmissionSetup(const FVehicleTransmissionData& NewTransmissionSetup)
{
#if WITH_VEHICLE
	if (PVehicleDrive)
	{
		PxVehicleGearsData GearData;
		GetVehicle12WGearSetup(NewTransmissionSetup, GearData);

		PxVehicleAutoBoxData AutoBoxData;
		GetVehicle12WAutoBoxSetup(NewTransmissionSetup, AutoBoxData);
