...
UWheeledVehicleMovementComponent10W::UWheeledVehicleMovementComponent10W(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	// Initialize WheelSetups array with 10 wheels
	WheelSetups.SetNum(10);
}

#if WITH_EDITOR
void UWheeledVehicleMovementComponent10W::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
...
static void GetVehicle10WEngineSetup(const FVehicleEngineData& Setup, PxVehicleEngineData& PxSetup)
{
...
static void GetVehicle10WGearSetup(const FVehicleTransmissionData& Setup, PxVehicleGearsData& PxSetup)
{
...
static void GetVehicle10WAutoBoxSetup(const FVehicleTransmissionData& Setup, PxVehicleAutoBoxData& PxSetup)
{
	for (int32 i = 0; i < Setup.ForwardGears.Num(); i++)
	{
...
void SetupDriveHelper(const UWheeledVehicleMovementComponent10W* VehicleData, const PxVehicleWheelsSimData* PWheelsSimData, PxVehicleDriveSimDataNW& DriveData)
{

	PxVehicleEngineData EngineSetup;
	GetVehicle10WEngineSetup(VehicleData->EngineSetup, EngineSetup);
	...
	GetVehicle10WGearSetup(VehicleData->TransmissionSetup, GearSetup);
	DriveData.setGearsData(GearSetup);

	PxVehicleAutoBoxData AutoBoxSetup;
	GetVehicle10WAutoBoxSetup(VehicleData->TransmissionSetup, AutoBoxSetup);
	DriveData.setAutoBoxData(AutoBoxSetup);

}

static void CreateVehicleSimulation10W(PxVehicleWheelsSimData& Veh10WSimData, int32 NumWheels, PxVehicleDriveSimDataNW& DriveSimData10W)
{
	PxVehicleWheelsSimData* PWheelsSimData = PxVehicleWheelsSimData::allocate(4);
	Veh10WSimData.copy(*PWheelsSimData, 0, 0);
	Veh10WSimData.copy(*PWheelsSimData, 1, 1);
	Veh10WSimData.copy(*PWheelsSimData, 2, 2);
	Veh10WSimData.copy(*PWheelsSimData, 3, 3);
	//Copy front wheels bones to apply throttle and make rest of the wheels physical
	Veh10WSimData.copy(*PWheelsSimData, 0, 4);
	Veh10WSimData.copy(*PWheelsSimData, 1, 5);
	Veh10WSimData.copy(*PWheelsSimData, 0, 6);
	Veh10WSimData.copy(*PWheelsSimData, 1, 7);
	Veh10WSimData.copy(*PWheelsSimData, 0, 8);
	Veh10WSimData.copy(*PWheelsSimData, 1, 9);
	Veh10WSimData.setTireLoadFilterData(PWheelsSimData->getTireLoadFilterData());
	...
	DriveSimData10W.setDiffData(DifData);
}

void UWheeledVehicleMovementComponent10W::SetupVehicle()
{
	...
	// Setup the wheels
	PxVehicleWheelsSimData* PWheelsSimData = PxVehicleWheelsSimData::allocate(10);

	// Setup drive data
	PxVehicleDriveSimDataNW DriveData;
	CreateVehicleSimulation10W(*PWheelsSimData, 10, DriveData);

	SetupWheels(PWheelsSimData);
	SetupDriveHelper(this, PWheelsSimData, DriveData);

	PxVehicleDriveNW* PVehicleDriveNW = PxVehicleDriveNW::allocate(10);
...
void UWheeledVehicleMovementComponent10W::UpdateSimulation(float DeltaTime)
{
	...
	TArray<FRichCurveKey> SteerKeys = SteeringCurve.GetRichCurve()->GetCopyOfKeys();
	const int32 MaxSteeringSamples = FMath::Min(10, SteerKeys.Num());
...
void UWheeledVehicleMovementComponent10W::UpdateEngineSetup(const FVehicleEngineData& NewEngineSetup)
{
#if WITH_VEHICLE
	if (PVehicleDrive)
	{
		PxVehicleEngineData EngineData;
		GetVehicle10WEngineSetup(NewEngineSetup, EngineData);
...
void UWheeledVehicleMovementComponent10W::UpdateTransmissionSetup(const FVehicleTransmissionData& NewTransmissionSetup)
{
#if WITH_VEHICLE
	if (PVehicleDrive)
	{
		PxVehicleGearsData GearData;
		GetVehicle10WGearSetup(NewTransmissionSetup, GearData);

		PxVehicleAutoBoxData AutoBoxData;
		GetVehicle10WAutoBoxSetup(NewTransmissionSetup, AutoBoxData);
